...
const Checkout = () => {
    ...
    const fetchCartItems = async () => {
        ...
        try {
            const response = await axios.get(`/api/cart/items/${userData.id}`);
            ...
            const updatedCartItems = await Promise.all(
                response.data.map(async item => {
                    const productResponse = await axios.get(`/api/imgProducts/${item.product.id}`);
    ...
    const handleProvinceChange = (e) => {
        ...
        axios.get(`api/districts/${selectedProvince}`)
            .then(response => {
            ...
            try {
                ...
                if (orderResponse.status === 201) {
                    ...
                    const promises = cartItems.map(async (item) => {
                        const orderDetailResponse = await axios.post('api/orderDetail', {
                            orderId: { id: orderId },
                        ...
                        return axios.delete(`api/cart/clear/user/${userData.id}`);
                    });
    ...
    const handleApplyDiscount = async () => {
        try {
            ...
            const response = await axios.get(`api/discount/${userData.id}/${discountCode}`)
