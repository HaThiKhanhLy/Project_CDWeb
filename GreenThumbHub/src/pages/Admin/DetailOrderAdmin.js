...
import { Descriptions, Tag, Divider, Button, Steps, Popover } from 'antd';
...
const DetailOrder = () => {
  ...
  const fetchOrderDetail = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/orderDetail/${id}`);
  ...
  const updateStatusOrder = async (statusId) => {
    try {
      const response = await axios.put(`http://localhost:8080/api/orders/updateStatus/${order.id}?statusId=${statusId}`);
